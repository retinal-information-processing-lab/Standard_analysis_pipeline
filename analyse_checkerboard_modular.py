...
def load_checkerboard_experiment_data(
    params: dict,
    check_directory: str,
    nb_checks_x: int,
    nb_checks_y: int,
...
def plot_all_cells_rasters(raster_data: dict, cells_id: list, plotting: bool = True):
    ...
    for i in tqdm(range(size**2)):
        ...
        if i < len(cells_id):
            ...
            utils.plot_single_raster(ax, spike_trains)
            ax.set(
...
def plot_one_cell_raster_and_psth(
    raster_data: dict, cell_nb, params: dict, show=False
):
    """
    ...
    utils.plot_single_raster(axs[0], raster_data[cell_nb]["spike_trains"])
    utils.plot_single_psth_from_raster_data(axs[1], raster_data, cell_nb, params)
...
def plot_one_cell_3D_spike_triggered_average(
    ...
    """
    print(
        f"Total : {len(checkerboard_spikes.keys())} neurons found\n\nClusters id :\n{cells_id}\n"
    )
