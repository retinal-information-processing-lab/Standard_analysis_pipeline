...
def load_triggers_and_spikes_select_chirp_type(params: dict, old: bool):

    """ Load trigger times and load spikes.  Select chirp type
    ...
    #Input to select recording number
    ...
    #load triggers
    trig_data = utils.load_obj(os.path.normpath(os.path.join(triggers_directory,'{}_{}_triggers.pkl'.format(exp,rec))))
    ...
    return cells, spike_times,spike_trains, trig_data, stim_onsets, check_directory,CT_directory, old


def compute_chirp_rasters(cells, spike_times, stim_onsets, old = False,n_bins=800,n_bins_small=16000):

    """Compute the chirp rasters
    
    Args:
       

    Returns:
        #2026-01-22 RWD: add data types to all of these.  For now just listing what I see.
...
def plot_chirp_rasters(cells, cell_data, CT_directory, check_directory):
    ...
    Returns:
        No return values.  Just generates the plots
...
def select_and_save_cells_for_clustering(good_sta_cells,good_chirp_cells,CT_directory, params):
    ...
    Returns:
        #Need these all for modifying in next step
        selected_cells
        selected_cells_sta
    ...
    selected_cells, selected_cells_sta, selected_cells_chirp = cell_selection_for_clustering(cells, CT_directory_path = fig_directory,selected_cells_sta = selected_cells_sta, selected_cells_chirp = selected_cells_chirp )
...
def modify_cells_for_clustering(selected_cells_sta, selected_cells_sta_to_add,selected_cells_sta_to_remove,
                                selected_cells_chirp, selected_cells_chirp_to_add, selected_cells_chirp_to_remove,
                               remove_any_way,
                               CT_directory, params):

    """modify selected cells for clustering and save a new list of selected cells
    ...
    Returns:
       ...
       #Return these for now so they can be checked and as they may be used in later steps.
        selected_cells
        selected_cells_sta
        selected_cells_chirp
...
def run_cell_typing_AC(dist_thres, n_components_psth, n_components_sta_tc,
    cell_data, selected_cells, check_directory, sparse = False):

    """runs cell typing: Agglomerative Clustering
...
def save_cluster_number_for_cells(selected_cells,cell_data,model):

    """Saves each cell's cluster number in the chirp response dictionary
    ...
    Returns:
       ...
       cell_data updated with this information

        


    Note:
        Run after your cell typing via agglomerative clustering.
...
def compute_intracluster_crosscorr(cell_data,sta_results,n_bins, max_shift,old):
    
    """Computes each cluster's crosscorr between cells in same cluster
...
def create_cluster_summary_figure(cell_data, psth_z,sta_results, params, CT_directory, old):

    """creates a figure summarizing each cluster.
    ...
    Returns:
       ...
       No returns.  Saves figures to a figure directory.
    ...
    exp=params.exp
    DG_set = utils.load_obj(os.path.join(utils.find_Analysis_Directory(dir_type="DG"),f'DG_data_exp{exp}'))
    ...
    for icluster in tqdm(range(len(list(set([cell_data[cell]["type"] for cell in cell_data.keys() if cell_data[cell]["type"] != 'Not assigned']))))[:]):
        idx_cluster = sorted(list(np.where(np.asarray([cell_data[cell]["type"] for cell in selected_cells if cell_data[cell]["type"] != 'Not assigned'])==icluster)[0]))
    ...
    #             ax.set_yticks([0,0.5,1])
            ...
            ax.plot([DG_set[cell_nb]['atune'],DG_set[cell_nb]['atune']],[0,DG_set[cell_nb]['Rtune']],'b-') #2026-01-23 RWD: changed third call of DG_set[cell] to DG_set[cell_nb] as getting error cell is not defined and all other calls have cell_nb
